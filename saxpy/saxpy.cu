...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    cudaHostAlloc((void**)&xarray, sizeof(float) * N, cudaHostAllocDefault);
    cudaHostAlloc((void**)&yarray, sizeof(float) * N, cudaHostAllocDefault);
    cudaHostAlloc((void**)&resultarray, sizeof(float) * N, cudaHostAllocDefault);
    ...
    printf("Effective BW by CUDA saxpy: [Total] %.3f ms, [Kernel] %.3f ms\t\t[%.3f GB/s]\n", 1000.f * overallDuration, 1000.f * kernelDuration, GBPerSec(totalBytes, overallDuration));
