...
__global__ void kernelRenderCircles() {
    ...
    if (index_x >= cuConstRendererParams.imageWidth || index_y >= cuConstRendererParams.imageHeight)
        return;

    for (int index_circle = 0; index_circle < cuConstRendererParams.numCircles; index_circle++) {
        const int index3 = 3 * index_circle;

        // read the position and radius of the circle
        float3 p = *(float3*)(&cuConstRendererParams.position[index3]);
        float rad = cuConstRendererParams.radius[index_circle];
        ...
        // if the pixel is within the bounding box of the circle, shade the pixel
        if (index_x >= screenMinX && index_x < screenMaxX && index_y >= screenMinY && index_y < screenMaxY) {
            ...
            shadePixel(index_circle, pixelCenterNorm, p, imgPtr);
        }
    }
}
